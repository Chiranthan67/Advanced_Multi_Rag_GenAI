...
class AdvanceRAG:
    ...
    def source_stats(self) -> dict[str, dict[str, int]]:
        ...
        for chunk in self.chunks:
            if chunk.source not in stats:
                stats[chunk.source] = {"chunks": 0, "text": 0, "image": 0}
            stats[chunk.source]["chunks"] += 1
            if chunk.modality == "image":
                stats[chunk.source]["image"] += 1
            else:
                stats[chunk.source]["text"] += 1
        return stats

    def remove_sources(self, sources: list[str]) -> int:
        if not sources:
            return 0
        source_set = set(sources)
        original = len(self.chunks)
        self.chunks = [chunk for chunk in self.chunks if chunk.source not in source_set]
        removed = original - len(self.chunks)
        if removed == 0:
            return 0
        if self.chunks:
            self.build_index()
